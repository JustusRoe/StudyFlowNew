document.addEventListener("DOMContentLoaded", function () {
  ...
  fetch(`/courses/details/${courseId}`)
    ...
    .then((course) => {
      document.getElementById("deadlineCourseInfo").innerHTML = `
                <h2>${course.name}</h2>
                <div>Difficulty: ${
                  ["Easy", "Medium", "Hard"][course.difficulty - 1] || "Unknown"
                }</div>
      ...
      document.getElementById(
        "progressLabel"
      ).textContent = `${course.progressPercent}%`;
    });
  ...
  function loadDeadlines() {
    fetch(`/api/courses/${courseId}/deadlines`)
      ...
      .then((deadlines) => {
        ...
        selector.innerHTML =
          '<option value="" disabled selected>Select a deadline</option>';
        if (!Array.isArray(deadlines) || deadlines.length === 0) {
          tbody.innerHTML =
            '<td class="empty-row" colspan="4">No deadlines yet.</td>';
          return;
        }
        deadlines.forEach((dl) => {
          ...
          tr.querySelector(".delete-btn").onclick = () => {
            if (confirm("Delete this deadline?")) {
              fetch(`/deadlines/delete/${dl.id}`, { method: "DELETE" }).then(
                () => loadDeadlines()
              );
            }
          };
          ...
          opt.value = dl.id;
          opt.textContent = `${dl.title} (${(dl.startTime || "").slice(
            0,
            16
          )})`;
  ...
  document.getElementById("deadlineForm").onsubmit = function (e) {
    ...
    const studyTimeNeeded = parseInt(
    ...
    fetch(`/deadlines/create?courseId=${courseId}`, {
      ...
      .catch((err) =>
        alert("Fehler beim Erstellen der Deadline: " + err.message)
      );
  };
    ...
    fetch(`/api/courses/${courseId}/selfstudy`)
      ...
      .then((events) => {
        ...
        if (!Array.isArray(events) || events.length === 0) {
          list.innerHTML =
            '<li class="empty-session">No planned self-study sessions yet.</li>';
          return;
        }
        events.forEach((ev) => {
          const deadlineInfo = ev.relatedDeadlineTitle
            ? ` [${ev.relatedDeadlineTitle}]`
            : "";
          list.innerHTML += `<li>${ev.title} (${ev.startTime.slice(
            0,
            16
          )} - ${ev.endTime.slice(0, 16)})${deadlineInfo}</li>`;
        });
      });
  }
  loadSessions();
  ...
  document.getElementById("selfstudyForm").onsubmit = function (e) {
    ...
    const deadlineOption =
      deadlineSelector.options[deadlineSelector.selectedIndex];
    const deadlineTitle =
      deadlineOption && deadlineOption.value
        ? deadlineOption.textContent.split(" (")[0]
        : "";
