...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const calendar = new FullCalendar.Calendar(calendarEl, {
      ...
      fetch("/calendar/events")
        .then((response) => {
          if (!response.ok) {
            return response.text().then((text) => {
              ...
              failureCallback &&
                failureCallback("Server error: could not load events.");
              return [];
            });
          }
        ...
        .then((allEvents) => {
          if (!Array.isArray(allEvents)) {
            console.error(
              "calendar/events did not return an array:",
              allEvents
            );
            failureCallback &&
              failureCallback("Server error: could not load events.");
    ...
    eventContent: function (arg) {
      ...
      if (type === "lecture" || type === "self-study") {
        ...
        time.innerText = event.start.toLocaleTimeString([], {
          hour: "2-digit",
          minute: "2-digit",
        });
  ...
  function openAddEventSidebar(info) {
    ...
    saveBtn.onclick = function () {
      ...
      if (!title || !startTime || !endTime || !type) {
        alert(
          "Please fill in all required fields: Title, Start/End Time, and Type."
        );
        return;
      }
  ...
  function openEditEventSidebar(event) {
    ...
    document.getElementById("editEventStart").value = event.startStr.slice(
      0,
      16
    );
      ...
      .then((courses) => {
        ...
        function selectCourseForEvent(courses, eventId) {
          ...
          for (const course of courses) {
            if (Array.isArray(course.eventIds)) {
              if (
                course.eventIds.some((eid) => String(eid) === String(eventId))
              ) {
            ...
            if (dropdown) {
              if (foundCourseId) {
                const exists = Array.from(dropdown.options).some(
                  (opt) => String(opt.value) === String(foundCourseId)
                );
                if (exists) {
  ...
  document.addEventListener("keydown", function (event) {
    if (event.key === "Escape") {
      ...
      if (editCourseSidebar && editCourseSidebar.classList.contains("open")) {
        ...
        document
          .querySelectorAll(".sidebar-tabs .tab-btn")
          .forEach((btn) => btn.classList.remove("active"));
        const overviewBtn = document.querySelector(
          '.sidebar-tabs .tab-btn[data-tab="overview"]'
        );
    ...
    fetch("/courses/user")
      ...
      .then((courses) => {
        ...
        courses.forEach((course) => {
          ...
          li.style.background = `linear-gradient(45deg, ${
            course.color || "#eeeeee"
          }, #0018ff9c)`;
  ...
  // Save and Cancel buttons in create course sidebar
  ...
  if (saveCourseBtn) {
    saveCourseBtn.onclick = function () {
      ...
      const color = colorInput.value || "#4287f5";
      if (!name) {
  ...
  if (cancelCourseBtn) {
    cancelCourseBtn.onclick = function () {
      sidebar.classList.remove("open");
      const nameInput = document.getElementById("courseName");
      const colorInput = document.getElementById("courseColor");
      if (nameInput) nameInput.value = "";
      if (colorInput) colorInput.value = "#4287f5";
    };
  }
  ...
  window.openCourseDetailSidebar = function (courseId) {
    ...
    fetch(`/courses/details/${courseId}`)
      ...
      .then((course) => {
        ...
        const overviewColorBox = document.getElementById(
          "overviewCourseColorBox"
        );
        if (overviewColorBox)
          overviewColorBox.style.background = course.color || "#4287f5";
        const overviewColor = document.getElementById("overviewCourseColor");
        if (overviewColor) overviewColor.textContent = course.color || "-";
        const overviewDiff = document.getElementById(
          "overviewCourseDifficulty"
        );
        if (overviewDiff)
          overviewDiff.textContent =
            ["Easy", "Medium", "Hard"][course.difficulty - 1] || "-";
        const overviewProgressBar = document.getElementById(
          "overviewCourseProgressBar"
        );
        if (overviewProgressBar)
          overviewProgressBar.style.width = `${progress}%`;
        const overviewProgressLabel = document.getElementById(
          "overviewCourseProgressLabel"
        );
        if (overviewProgressLabel)
          overviewProgressLabel.textContent = `${progress}%`;
        ...
        const colorSettings = document.getElementById(
          "editCourseColorSettings"
        );
        if (colorSettings) colorSettings.value = course.color || "#4287f5";
        const diffSettings = document.getElementById(
          "editCourseDifficultySettings"
        );
  ...
  function loadCourseLectures(courseId) {
    fetch(`/courses/events/${courseId}`)
      ...
      .then((events) => {
        ...
        events
          .filter((ev) => ev.type && ev.type.toLowerCase() === "lecture")
          .forEach((ev) => {
            const tr = document.createElement("tr");
            tr.innerHTML = `
                        <td>${ev.title}</td>
    ...
    btn.addEventListener("click", function () {
      ...
      if (this.dataset.tab === "settings") {
        ...
        if (document.getElementById("editCourseDescriptionSettings") && desc)
          document.getElementById("editCourseDescriptionSettings").value =
            desc.value;
        if (document.getElementById("editCourseColorSettings") && color)
          document.getElementById("editCourseColorSettings").value =
            color.value;
        if (document.getElementById("editCourseDifficultySettings") && diff)
          document.getElementById("editCourseDifficultySettings").value =
            diff.value;
      }
  ...
  window.updateCourse = function () {
    ...
    const description = document
      .getElementById("editCourseDescription")
      .value.trim();
  ...
  const closeCourseSidebarBtn = document.getElementById(
    "closeCourseSidebarBtn"
  );
  if (closeCourseSidebarBtn) {
    closeCourseSidebarBtn.addEventListener("click", window.closeSidebar);
  }
  const closeEditCourseSidebarBtn = document.getElementById(
    "closeEditCourseSidebarBtn"
  );
  if (closeEditCourseSidebarBtn) {
    closeEditCourseSidebarBtn.addEventListener(
      "click",
      window.closeEditSidebar
    );
  }

  // Attach delete handler for deleteCourseButtonOverview to delete course
  const deleteBtnOverview = document.getElementById(
    "deleteCourseButtonOverview"
  );
